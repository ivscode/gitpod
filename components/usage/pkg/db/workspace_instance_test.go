...
func TestListWorkspaceInstancesInRange(t *testing.T) {
	...
	tx := conn.Session(&gorm.Session{AllowGlobalUpdate: true}).Delete(&db.WorkspaceInstance{})
	require.NoError(t, tx.Error)
	...
	require.EqualValues(t, valid, retrieved)
	//require.Len(t, retrieved, 6)
}
